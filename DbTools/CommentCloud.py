...
def cut_word(commentsFile):  # 对数据分词
    with open(commentsFile, encoding='utf-8') as f:
        content_txt = f.read()
        word_list = jieba.lcut(content_txt.strip(), cut_all=True)
        #print(word_list)
        dic = {}
...
def create_word_cloud(commentsFile):  # 生成词云
    ...
    wc = WordCloud(font_path='C:/Windows/Fonts/SIMLI.TTF',   # 设置字体为本地的字体，有中文必须要加
                   ...
                   color_func=random_color_func).generate_from_text(cut_wordByList(commentsFile))  # 设置词云的一些配置,有mask参数再设定宽高是无效的
...
def create_word_cloud_fq(commentsFile):  # 生成词云
    ...
    wc = WordCloud(font_path='C:/Windows/Fonts/SIMLI.TTF',   # 设置字体为本地的字体，有中文必须要加
                   ...
                   random_state=40,     # 设置有多少种随机生成状态，即有多少种配色方案
                   mask=background_image,
                   color_func=random_color_func).generate_from_frequencies(cut_wordByList(commentsFile))  # 设置词云的一些配置,有mask参数再设定宽高是无效的
    wc.to_file('wordc.jpg')  # 保存图片
    plt.imshow(wc, interpolation='bilinear')
    plt.axis('off')  # 不显示坐标轴
    plt.show()
...
#     create_word_cloud_fq(['这件东西不错','中国标准时间','正在爬取商品'])
