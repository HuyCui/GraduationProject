...
def getPage_link(keyword):
    ...
    searchUrl = 'http://search.jd.com/Search?keyword='+keyword+'&psort=2&click=0'
...
def getOtherLink(keyword, page):
    ...
    for i in range(page):
        ...
        url = 'http://search.jd.com/Search?keyword=%s&psort=2&psort=2&page=%d&s=60&click=0' %(keyword, 2*i+3)
        headers = {
...
def getAllGoodsComments(keyword, goodsId):
    ...
    # for goodId in goodsId:
    #     maxPage = 13
    #     result = getOneGoodComment(keyword, goodId, maxPage)
    #     parse_comments(result)


if __name__ == '__main__':

    goodsId1 = getPage_link('洗衣粉')
    goodsId2 = getOtherLink('洗衣粉', 19)
    goodsId1.extend(goodsId2)
    print('已获得商品的id，准备开始爬取评论。。。')
    print(goodsId1)
    time.sleep(10)

    getAllGoodsComments('洗衣粉', goodsId1)


    time.sleep(605)

    goodsId1 = getPage_link('牙刷')
    goodsId2 = getOtherLink('牙刷', 19)
    goodsId1.extend(goodsId2)
    print('已获得商品的id，准备开始爬取评论。。。')
    print(goodsId1)
    time.sleep(10)

    getAllGoodsComments('牙刷', goodsId1)
